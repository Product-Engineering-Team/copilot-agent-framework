...
NO-CODE ──► SIMPLE ──► CROSS-SERVICE ──► ARCHITECTURAL
  │              │              │                  │
  │         1 file/function     │        Schema or contract change
...
### Definition
A logic change isolated to a single file or function that does not affect
any shared layer, API contract, or database schema.
...
### Agent Required
Domain agent only — Backend Agent or Frontend Agent depending on the layer.
...
but does not require modifying an existing DB schema or API contract.
...
### Agent Required
Domain agents (Backend + Frontend) with Architect Agent review.
Architect Agent must confirm no contract change is needed before implementation begins.
...
| Refactor auth middleware to support a new role type | Shared middleware + multiple routes |
...
- More than 2 teams or services need coordination → **Architectural**
...
or requires a fundamental design decision that affects the long-term structure
of the system.
...
- Any decision that cannot be reversed without a migration

### Agent Required
Architect Agent MUST go first and produce a complete spec.
No Backend or Frontend agent starts work until architecture.md is updated.
...
Does it require writing or changing any code?
  ...
  └── YES
        │
        ▼
      Does it modify a DB schema or API contract?
        │
        ├── YES ─────────────────────────► ARCHITECTURAL
        │
        └── NO
